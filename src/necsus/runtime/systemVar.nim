...
proc unwrap*[T](sysvar: SharedOrT[T] | LocalOrT[T], otherwise: T): T {.inline.} =
  ...
  return
    when sysvar is T:
      ...
      sysvar.get(otherwise)

proc `$`*[T](sysvar: SystemVar[T]): string =
  $sysvar.extract.value
