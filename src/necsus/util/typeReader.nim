...
proc isPragma*(found, expect: NimNode): bool =
  ...
  of nnkSym:
    return found == expect
  of nnkCall:
    return found[0].isPragma(expect)
  of nnkIdent:
    return false
  of nnkExprColonExpr:
    return found[0].isPragma(expect)
  else:
    error("Unable to extract pragma from " & found.lispRepr, found)
...
proc asGenericTable(
    genericParams: NimNode, values: openArray[NimNode]
): Table[string, NimNode] =
