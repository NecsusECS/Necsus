...
type
  ...
  ParsedSystem* = ref object ## Parsed information about a system proc
    id*: int
    phase*: SystemPhase
    symbol*: NimNode
...
proc `$`*(system: ParsedSystem): string =
  ...
  let instancedStr =
    if system.instanced.isSome: system.instanced.get.lispRepr else: "none"
  &"{system.symbol}(" &
    join(
      [
        $system.phase,
...
proc parseArgKind(symbol: NimNode): Option[DirectiveGen] =
  ...
  case symbol.strVal
  of "Spawn":
    return some(spawnGenerator)
  of "FullSpawn":
    return some(fullSpawnGenerator)
  of "Query":
    return some(queryGenerator)
  of "FullQuery":
    return some(fullQueryGenerator)
  of "Attach":
    return some(attachGenerator)
  of "Detach":
    return some(detachGenerator)
  of "Shared":
    return some(sharedGenerator)
  of "Local":
    return some(localGenerator)
  of "Lookup":
    return some(lookupGenerator)
  of "Inbox":
    return some(inboxGenerator)
  of "Outbox":
    return some(outboxGenerator)
  of "TimeDelta":
    return some(deltaGenerator)
  of "TimeElapsed":
    return some(elapsedGenerator)
  of "EntityDebug":
    return some(entityDebugGenerator)
  of "Bundle":
    return some(bundleGenerator)
  of "Delete":
    return some(deleteGenerator)
  of "DeleteAll":
    return some(deleteAllGenerator)
  of "TickId":
    return some(tickIdGenerator)
  of "Save":
    return some(saveGenerator)
  of "Restore":
...
proc parseDirectiveArgsFromTuple(tupleArg: NimNode): seq[DirectiveArg] =
  ...
  for child in tupleArg.getTupleSubtypes():
...
proc parseParametricArg(
    context, argName, directiveSymbol: NimNode, directiveParametrics: openarray[NimNode]
): Option[SystemArg] =
...
proc parseArgType(context, argName, argType, original: NimNode): SystemArg =
  ...
  case argType.kind
  of nnkBracketExpr:
    parsed = parseParametricArg(context, argName, argType[0], argType[1 ..^ 1])
  of nnkCall:
...
proc parseActiveCheck(context, state: NimNode): seq[ActiveCheck] =
  ...
  of nnkCurly:
    ...
    if typename.kind != nnkSym:
      error(
        "Unexpected system state type. Expecting a symbol, but got " & typename.repr,
        state,
      )
...
proc hasInstancedReturnType(node: NimNode): bool =
  ...
  of nnkSym:
    return
      node == bindSym("SystemInstance") or node == bindSym("EventSystemInstance") or
      node == bindSym("SaveSystemInstance")
  of nnkBracketExpr:
    return node[0].hasInstancedReturnType
  of nnkProcTy:
    return node.params[0].hasInstancedReturnType
  of nnkProcDef:
    return node[0].getTypeImpl.hasInstancedReturnType
  of nnkIdentDefs, nnkPragmaExpr:
    return node[0].findSym.getTypeImpl.hasInstancedReturnType
  else:
    return false
...
proc determineInstancing(nodeImpl: NimNode, nodeTypeImpl: NimNode): Option[NimNode] =
  ...
  if nodeImpl.hasInstancedReturnType():
    let tupleTy =
      nodeTypeImpl.params[0].resolveTo({nnkProcTy}).get(nodeTypeImpl.params[0])
    return some(tupleTy)
...
proc getSystemType(ident: NimNode, impl: NimNode): NimNode =
  ...
  of nnkIdentDefs, nnkProcDef:
    return impl[1].resolveTo({nnkProcTy}).orElse:
      ident.getTypeImpl
  of nnkLambda:
    return impl
  else:
...
proc parseApp*(appProc: NimNode, runner: NimNode): ParsedApp =
  ...
  result = ParsedApp(
    ...
    returns:
      if returnNode.kind == nnkEmpty:
        none(MonoDirective)
      else:
        some(newMonoDir(returnNode)),
  )
...
proc instancedInfo*(
    system: ParsedSystem
): Option[tuple[fieldName: NimNode, typ: NimNode]] =
