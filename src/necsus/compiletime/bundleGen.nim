...
proc generate(
  ...
  of Late:
    ...
    for nested in arg.nestedArgs:
      construct.add(
        nnkExprColonExpr.newTree(ident(nested.originalName), details.systemArg(nested))
      )
...
proc nestedArgs(dir: MonoDirective): seq[RawNestedArg] =
  ...
  for child in impl[2].children:
    ...
    result.add((dir.argType, name, child[1]))
