import
  ...
  tickGen,
  tools,
  std/[sequtils, tables, json, jsonutils, sets]

proc saveTypeName(genInfo: CodeGenInfo): NimNode =
  ident(genInfo.app.name & "SaveMarshal")

proc restoreTypeName(genInfo: CodeGenInfo): NimNode =
  ident(genInfo.app.name & "RestoreMarshal")

proc restoreSysType(sys: ParsedSystem): NimNode =
  ## Returns the type that a restoreSys accepts for restoration
  sys.prefixArgs[0][1]

proc collectMarshalTypes(
    topics: openArray[NimNode], dedupe: var HashSet[string], records: var NimNode
) =
  ...
  for topicType in topics:
    ...
    if topicName in seen:
      ...
      error(
        "A type named '" & topicName &
          "' is already returned by a different marshaling system",
        topicType,
      )
    else:
...
proc createMarshalType(
    genInfo: CodeGenInfo, typeName: NimNode, includeRestore: bool
): NimNode =
...
proc createRestoreProc(genInfo: CodeGenInfo): NimNode =
  ...
  return quote:
    proc restore*(
        `appStateIdent`: ptr `appStateType`, `streamIdent`: string
