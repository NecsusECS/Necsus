...
import macros, sequtils, tables, strutils
...
proc calculateArchetypes(
    allSystems: openarray[ParsedSystem], runnerArgs: seq[SystemArg]
...
proc calculateDirectives(
    args: openarray[SystemArg]
): Table[DirectiveGen, DirectiveSet[SystemArg]] =
...
proc newCodeGenInfo*(
    config: NimNode, app: ParsedApp, allSystems: openarray[ParsedSystem]
): CodeGenInfo =
  ## Collects data needed for code gen from all the parsed systems
...
proc generateForHook*(
    codeGen: CodeGenInfo, system: ParsedSystem, hook: GenerateHook
): NimNode =
