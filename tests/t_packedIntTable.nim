import unittest, necsus/util/packedIntTable, sequtils, options
...
suite "PackedIntTable":
    ...
    test "Filling the table beyond capacity":
        ...
        for i in 0'i32..size:
            check(fill[i] == i)
            check(fill.getPointer(i)[] == i)
            check(fill.maybeGet(i).get == i)
            check(fill.maybeGetPointer(i).get[] == i)
            check(fill[fill.getRef(i)] == i)
            check(i in fill)

        check(fill.toSeq == (0..size).toSeq)

    test "Take address of a key":
        let ref1 = table.getRef(1)
        let ref2 = table.getRef(2)
        let ref3 = table.getRef(3)
