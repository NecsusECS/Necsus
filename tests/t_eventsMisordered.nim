...
proc receive1(receive: Inbox[int]) =
    case timesThrough
    of 0: check(receive.items.toSeq == newSeq[int]())
    of 1: check(receive.items.toSeq == @[0, 50])
    of 2: check(receive.items.toSeq == @[1, 51])
    else: assert(false)
...
proc receive2(receive: Inbox[int]) =
    case timesThrough
    of 0: check(receive.items.toSeq == @[0])
    of 1: check(receive.items.toSeq == @[50, 1])
    of 2: check(receive.items.toSeq == @[51, 2])
    else: assert(false)
