...
proc setup(
    spawnAB: FullSpawn[(A, B)], spawnABC: Spawn[(A, B, C)], attachC: Attach[(C, D)]
) {.startupSys.} =
  for i in 1 .. 3:
...
proc assertions(query: Query[(A, B, Option[C], Option[D])]) =
  ...
  check(
    query.items.toSeq.filterIt(it[2].isSome).mapIt(it[2].get().c).toHashSet ==
      [1, 11, 2, 12, 3, 13].toHashSet
  )
  check(query.items.toSeq.filterIt(it[2].isNone).len == 3)
  check(
    query.items.toSeq.filterIt(it[3].isSome).mapIt(it[3].get().d).toHashSet ==
      [51, 52, 53].toHashSet
  )
